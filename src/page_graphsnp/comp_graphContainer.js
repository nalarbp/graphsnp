...
const GraphContainer = (props) => {
  ...
  const graph_isUserCompoundNodeRelayout = false;
  ...
  useEffect(() => {
    if (graph_isUserCompoundNodeRelayout && cytoscapeRef.current) {
      setTimeout(() => {
        let cy = cytoscapeRef.current;
        //redo graph with compound node
        cytoscapeRef.current = cy;
        props.isShowingLoadingModalToStore(false);
        //props.changeIsUserCompoundNodeRelayout(false);
      }, 100);
    }
  }, [graph_isUserCompoundNodeRelayout]);
        ...
        const cy = cytoscape({
          ...
          style: [
            ...
            },
            {
              ...
              style: {
                // add background image by svg string generated by createSVGStringFromObject function
                // "background-image": function (d) {
                //   //let svgString = createSVGStringFromObject("nodeData");
                //   return "svgString";
                // },
                "padding-top": "0px",
